(() => {
  ...
  class Ribbon {
    constructor(si, st) {
      ...
      this.pts = []
      this.hue = Math.floor((Math.random() - 0.5) * 0.2)

      // min 2 points
      this.pts.push({ x: canvas.width * this.offset + this.step, y: 0 })
      this.pts.push({ x: canvas.width * this.offset - this.step, y: 0 })

      for (let i = 0; i < this.numPts - 2; i++) {
        this.pts.push({ x: (canvas.width * this.offset) + (this.step * (Math.random() - 0.5)), y: 0 })
      }
    }
  ...
  window.addEventListener("scroll", e => {
    if (!fired && window.scrollY > window.innerHeight / 2) {
      init()
      fired = true
    }
  })
